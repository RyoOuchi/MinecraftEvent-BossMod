...
public class BossRemovalReasonHandler {
    @SubscribeEvent
    public static void onBossRemoved(EntityLeaveWorldEvent event) {
        final Entity entity = event.getEntity();
        final Level level = entity.level;
        if (!(entity.getType().equals(ImportantConstants.BOSS_ENTITY_TYPE))) return;

        final Entity.RemovalReason removalReason = event.getEntity().getRemovalReason();

        if (removalReason != Entity.RemovalReason.DISCARDED) {
            return;
        }
        summonBossAgain(level);
    }

    private static void summonBossAgain(Level level) {
